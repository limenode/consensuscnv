...
#table(
  ...
  table.hline(stroke: 0.6pt),
  [30x], [CNVpytor], [29,578], [13,419], [87], [16,159], [13,003], [384], [119], [16,575],
  [], [Delly], [7,453], [428], [0], [7,025], [421], [0], [7], [7,032],
  [], [GATK-gCNV], [13,795], [5,579], [0], [8,216], [5,563], [16], [0], [8,232],
  table.hline(stroke: 0.3pt),
  [6x], [CNVpytor], [9,327], [847], [39], [8,480], [644], [122], [120], [8,683],
  [], [Delly], [4,324], [217], [1], [4,107], [209], [1], [8], [4,115],
  [], [GATK-gCNV], [8,534], [23], [0], [8,511], [23], [0], [0], [8,511],
  table.hline(stroke: 0.3pt),
  [4x], [CNVpytor], [8,144], [844], [41], [7,300], [639], [125], [121], [7,505],
  [], [Delly], [3,840], [101], [0], [3,739], [93], [0], [8], [3,747],
  [], [GATK-gCNV], [5,052], [17], [11], [5,035], [15], [13], [0], [5,037],
  table.hline(stroke: 0.3pt),
  [2x], [CNVpytor], [7,080], [626], [47], [6,454], [430], [125], [118], [6,650],
  [], [Delly], [3,138], [98], [0], [3,040], [93], [0], [5], [3,045],
  [], [GATK-gCNV], [10,561], [8], [9], [10,553], [7], [10], [0], [10,554],
  table.hline(stroke: 0.6pt),
)
]

#cap("Supplementary Table Exclusion Accounting:")[
  Effect of the excluded-region mask on each caller's calls at each coverage, under the strict criterion (a call is removed when more than 1% of it lies in excluded regions) and the adopted one (removed when more than 50% does, with the ends of retained calls trimmed back to the edge of any excluded region they fall in, so that no call is split).
  Raw Calls are the calls parsed from each caller's output for the 13 samples before masking.
  Dropped are removed; Trimmed are retained with one or both ends trimmed; Overlap are retained unchanged while sharing sequence with the mask, which under the adopted criterion means only excluded regions lying wholly inside the call; Kept are all calls retained.
  Raw Calls minus Dropped equals Kept.
  Generated by `exclusion_accounting.py`.
]
...
#table(
  ...
  table.hline(stroke: 0.6pt),
  [30x], [1/3], [43,112], [18,808], [0], [0], [0], [2], [24,302], [23,876],
  [], [2/3], [5,004], [154], [3], [0], [0], [−2], [4,849], [4,836],
  [], [3/3], [2,402], [2], [0], [0], [0], [0], [2,400], [2,400],
  table.hline(stroke: 0.3pt),
  [6x], [1/3], [19,507], [858], [0], [0], [0], [0], [18,649], [18,438],
  [], [2/3], [1,721], [13], [0], [0], [0], [0], [1,708], [1,708],
  [], [3/3], [688], [0], [0], [0], [0], [0], [688], [688],
  table.hline(stroke: 0.3pt),
  [4x], [1/3], [15,267], [717], [0], [0], [0], [0], [14,550], [14,335],
  [], [2/3], [1,148], [29], [0], [0], [0], [0], [1,119], [1,119],
  [], [3/3], [309], [1], [0], [0], [0], [0], [308], [308],
  table.hline(stroke: 0.3pt),
  [2x], [1/3], [19,614], [505], [0], [0], [0], [0], [19,109], [18,907],
  [], [2/3], [746], [25], [0], [0], [0], [0], [721], [721],
  [], [3/3], [177], [0], [0], [0], [0], [0], [177], [177],
  table.hline(stroke: 0.6pt),
)
]

#cap("Supplementary Table Exclusion Consensus:")[
  Consensus call sets built with and without the excluded-region mask, at the adopted 50% reciprocal overlap and above the 1 kb size floor.
  No Mask is the number of consensus components built from unmasked calls; Masked (adopted criterion) and Strict (1% criterion) from masked calls.
  Each masked component was traced to the unmasked components its calls came from, and the difference under the adopted criterion is decomposed exactly: No Mask − Vanished − Lost Caller − Below Floor + Gained − Split/Merge = Masked.
  Vanished components lost every member call; Lost Caller components kept some members but no longer meet the agreement level; Below Floor components were trimmed under 1 kb; Gained components meet the level only after masking; Split/Merge is the net change from components that divided or joined when a member was removed or trimmed.
  The 1/3 counts are smaller than the component counts of Supplementary Table Callsets, which are given before the size floor.
  Generated by `exclusion_accounting.py`.
]
